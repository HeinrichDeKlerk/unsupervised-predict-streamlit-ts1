...
def collab_model(movie_list,top_n=10):
    ...
    for x in movie_list:
        ...
        for i in set(users_df['userId']):
            ...
            users_df = users_df.append(pd.Series([int(i),int(mid),est], index=['userId','movieId','rating']), ignore_index=True)
    ...
    cosine_sims = pd.DataFrame(sims, index = users_df['movieId'].values.astype(int), columns = users_df['movieId'].values.astype(int))
